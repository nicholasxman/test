...
import random
import uuid
#ceccccccccccccccccccccccccccccccccc
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!再加上定时
MY_USER_AGENT = [
...
# 打开数据库连接
conn = pymssql.connect(host="(local)", database="yuanchao", charset='utf8')

# 打开数据库连接
conn2 = pymysql.connect(host="192.168.232.132", database="yuanchao", port=3306,  user="root", password="123456", charset='utf8')
...
sql1 = '''  select  f1  FROM yuanchao..TaiZhou_Project    '''
cur.execute(sql1)
...
for project_name in project_names:
    ...
    res = requests.get(baseurl,params=params,headers={ 'User-Agent': head} ,      timeout=10)
    res.encoding = 'utf-8'
    # print(res.text)
    print(res.encoding)
    ...
    for i in assessment_en[1:len(assessment_en)]   :
        ...
        fields=','.join(['%s']*7)
        sql2 = '''  insert into yuanchao.taizhou_test values(%s)   ''' % (fields)
        #使用execute()方法执行sql查询
        cur2.execute(sql2, ( uuid.uuid1(),  F1, F2,  F3, F4,F5,time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
        conn2.commit()
conn2.close()
